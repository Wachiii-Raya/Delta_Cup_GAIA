...
mycursor.execute(
    ...
    "tube_rotator1 BOOLEAN NOT NULL, centrifuge BOOLEAN NOT NULL, cen_rotator BOOLEAN NOT NULL, tube_rotator2 BOOLEAN NOT NULL, NaClO BOOLEAN NOT NULL)")
...
[mycursor.execute(f"INSERT INTO valve(base_buffer, acid_buffer, NaClO) VALUES (0,0)")]
[mycursor.execute(f"INSERT INTO motor(media, cell, agitator, bioreactor, tube_rotator1, centrifuge, cen_rotate, tube_rotator2, NaClO) VALUES (0,0,0,0,0,0,0,0,0)")]
db.commit()
...
def pump_to_tube():
    for round in range(2):
        for pos in range(1,13): #round 1 pos = 1-2 wait for placing
            ...
            # time.sleep(5)       #place_tube
            gv.motor_on(3)      #motor4 = bioreactor 
            [mycursor.execute(f"UPDATE motor SET bioreactor = {gv.motor[3]} WHERE id = '1'")]
            db.commit()
            time.sleep(5)   #t_add_to_tube
            gv.motor_off(3)
            [mycursor.execute(f"UPDATE motor SET bioreactor = {gv.motor[3]} WHERE id = '1'")]
            db.commit()

            # rotate tube_rotator1
            gv.motor_on(4)      #motor5 = tube_rotator1,
            [mycursor.execute(f"UPDATE motor SET tube_rotator1 = {gv.motor[4]} WHERE id = '1'")]
            db.commit()
            time.sleep(5)     #t_move_tube_rotator
            gv.motor_off(4)
            [mycursor.execute(f"UPDATE motor SET tube_rotator1 = {gv.motor[4]} WHERE id = '1'")]
            db.commit()

            # (6-axis) move tube to centrifuge process @take_tube position from tube_rotator1
            time.sleep(5)       #pick_tube       # time delay ตอนรอ 6-axis ขยับ

            # rotate centrifuge for 1 position....
            gv.motor_on(8)
            [mycursor.execute(f"UPDATE motor SET cen_rotator = {gv.motor[8]} WHERE id = '1'")]
            db.commit()
            time.sleep(5)    #t_rotate_centrifuge
            gv.motor_off(8)
            [mycursor.execute(f"UPDATE motor SET cen_rotator = {gv.motor[8]} WHERE id = '1'")]
            db.commit()
...
#     for pos_tube in range(4):
#         gv.motor_on(6)
#         [mycursor.execute(f"UPDATE motor SET tube_rotator2 = {gv.motor[6]} WHERE id = '1'")]
#         db.commit()
#         time.sleep(5)       #t_move_tube_rotator_to_origin
#         gv.motor_off(6)
#         [mycursor.execute(f"UPDATE motor SET tube_rotator2 = {gv.motor[6]} WHERE id = '1'")]
#         db.commit()
    ...
    for round2 in range(2):
        for pos_cen in range(1,13):
            ...
            time.sleep(5)       #t_delay_for_6axis
